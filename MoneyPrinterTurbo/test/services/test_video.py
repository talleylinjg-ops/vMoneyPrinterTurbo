...
class TestVideoService(unittest.TestCase):
    ...
    def test_generate_video_skips_every_bgm_source_when_volume_is_zero(self):
        """0 音量且无字幕时应直接 ffmpeg 混音，不解析 BGM、不走 MoviePy 重编码。"""
        test_cases = [
            ("random", None),
            ("custom", None),
            ("sonilo", "sonilo.m4a"),
        ...
        for bgm_type, bgm_override in test_cases:
            with self.subTest(bgm_type=bgm_type):
                ...
                with (
                    ...
                    patch.object(vd, "_mux_video_audio_with_ffmpeg") as mux,
                ):
                    result = vd.generate_video(
                        video_path="combined.mp4",
                        audio_path="voice.mp3",
    ...
    def test_concat_video_clips_prefers_stream_copy(self):
        """片段已统一编码时，拼接应直接 copy，避免再整段重压。"""
        ...
        self.assertEqual(self._concat_mode(command), "copy")
        self.assertNotIn("-c:v", command)
        self.assertEqual(run.call_count, 1)

    def test_concat_video_clips_falls_back_after_runtime_encoder_failure(self):
        """
        copy 失败后回退重编码；硬件编码器失败时再改用 libx264。
        """
        config.app["video_codec"] = "h264_nvenc"

        def fake_run(command, capture_output, text, check):
            mode = self._concat_mode(command)
            if mode == "copy":
                return types.SimpleNamespace(
                    returncode=1,
                    stdout="",
                    stderr="concat copy incompatible",
                )
            if mode == "h264_nvenc":
                return types.SimpleNamespace(
                    returncode=1,
        ...
        used_modes = [self._concat_mode(call.args[0]) for call in run.call_args_list]
        self.assertEqual(used_modes, ["copy", "h264_nvenc", "libx264"])
        self.assertIn("h264_nvenc", vd._runtime_disabled_video_codecs)
    ...
    def test_concat_video_clips_does_not_disable_codec_when_fallback_also_fails(self):
        ...
        def fake_run(command, capture_output, text, check):
            ...
            return types.SimpleNamespace(
                returncode=1,
                stdout="",
                stderr=f"{mode} cannot write output",
            )
    ...
    def _capture_source_ranges_for_clip_speed(
        ...
        *,
        ...
        def _fake_prepare_clip(**kwargs):
            source_ranges.append((kwargs["start_time"], kwargs["end_time"]))
            output_duration = min(
                (kwargs["end_time"] - kwargs["start_time"]) / kwargs["clip_speed"],
                kwargs["max_clip_duration"],
            )
            written_durations.append(output_duration)
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                ...
                patch.object(
                    vd,
                    "_prepare_clip_with_ffmpeg",
                    side_effect=_fake_prepare_clip,
                ),
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(vd, "AudioFileClip", return_value=_FakeAudioClip()):
                with patch.object(
                    ...
                    with patch.object(
                        vd, "_prepare_clip_with_ffmpeg", return_value=3.0
                    ) as prepare_mock:
    ...
    def test_prepare_clip_with_ffmpeg_builds_cover_filter(self):
        """无转场裁切应走 ffmpeg cover 缩放，而不是 MoviePy 逐帧重压。"""
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(vd.subprocess, "run", side_effect=fake_run) as run:
                duration = vd._prepare_clip_with_ffmpeg(
    ...
    def test_prepare_clip_with_ffmpeg_caps_output_duration_after_speed(self):
        """快放后的输出时长仍受 max_clip_duration 限制。"""
        ...
        self.assertIn("setpts=PTS/2.0", filter_arg)
        self.assertIn("fps=30", filter_arg)

    def test_combine_videos_uses_moviepy_for_fade_transition(self):
        """有转场时仍走 MoviePy 写片段，不能误入 ffmpeg 裁切路径。"""
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                patch.object(vd, "AudioFileClip", return_value=_FakeAudioClip()),
                patch.object(
                    vd, "_open_video_clip_quietly", return_value=_FakeVideoClip(8.0)
                ),
                patch.object(vd, "_prepare_clip_with_ffmpeg") as prepare_mock,
                patch.object(vd, "_write_videofile_with_codec_fallback") as write_mock,
                patch.object(
                    vd.video_effects, "fadein_transition", side_effect=lambda clip, _d: clip
                ) as fadein,
                patch.object(vd, "concat_video_clips_with_ffmpeg"),
                patch.object(vd, "delete_files"),
            ):
