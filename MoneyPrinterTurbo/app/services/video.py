...
def concat_video_clips_with_ffmpeg(
    ...
    def build_command(*, copy_streams: bool, codec: str | None = None) -> list[str]:
        command = [
        ...
        if copy_streams:
            # 片段已经在裁切阶段统一成相同分辨率/帧率/编码，拼接时直接 copy
            # 避免对 5 分钟成片再整段 libx264 重压。
    ...
    def run_concat(*, copy_streams: bool, codec: str | None = None):
        command = build_command(copy_streams=copy_streams, codec=codec)
        result = subprocess.run(
            command,
            capture_output=True,
            text=True,
        ...
        return "copy" if copy_streams else codec

    try:
        try:
            return run_concat(copy_streams=True)
        except Exception as copy_exc:
            logger.warning(
                f"ffmpeg concat copy failed, re-encoding clips: {copy_exc}"
            )
            effective_codec = _get_effective_video_codec()
            try:
                return run_concat(copy_streams=False, codec=effective_codec)
            except Exception as exc:
                if effective_codec == _DEFAULT_VIDEO_CODEC:
                    raise
                result_codec = run_concat(
                    copy_streams=False, codec=_DEFAULT_VIDEO_CODEC
                )
...
def _uses_moviepy_clip_pipeline(transition_value) -> bool:
    return transition_value not in (
        None,
        "",
        "None",
...
def _ffmpeg_clip_filter(
    ...
    filters = []
    if (int(source_width), int(source_height)) != (int(target_width), int(target_height)):
    ...
    if abs(float(clip_speed) - 1.0) > 1e-6:
        filters.append(f"setpts=PTS/{float(clip_speed)}")
    filters.append(f"fps={fps}")
    filters.append("setsar=1")
    return ",".join(filters)


def _prepare_clip_with_ffmpeg(
    *,
    source_path: str,
    start_time: float,
    end_time: float,
    ...
    source_duration = max(0.0, float(end_time) - float(start_time))
    if source_duration <= 0:
        raise ValueError("empty clip duration")
    speed = max(float(clip_speed), 0.01)
    output_duration = min(source_duration / speed, float(max_clip_duration))
    input_duration = min(source_duration, output_duration * speed)
    command = [
        utils.get_ffmpeg_binary(),
        "-y",
        "-ss",
    ...
    if result.returncode != 0:
        error_message = (result.stderr or result.stdout or "").strip()
        raise RuntimeError(error_message or "ffmpeg clip prepare failed")
...
def _mux_video_audio_with_ffmpeg(
    ...
    volume = 1.0 if voice_volume is None else float(voice_volume)
    ...
    if result.returncode != 0:
        error_message = (result.stderr or result.stdout or "").strip()
        raise RuntimeError(error_message or "ffmpeg mux failed")
...
def combine_videos(
    ...
    for i, subclipped_item in enumerate(subclipped_items):
        ...
        try:
            ...
            if not _uses_moviepy_clip_pipeline(transition_value):
                ...
                clip_duration_saved = _prepare_clip_with_ffmpeg(
...
def generate_video(
    ...
    need_subtitle = bool(
        params.subtitle_enabled and subtitle_path and os.path.exists(subtitle_path)
    )
    need_bgm = bgm_service.should_use_bgm(params.bgm_type, params.bgm_volume)
    if not need_subtitle and not need_bgm:
        logger.info("muxing video and audio with ffmpeg (no subtitle, no bgm)")
        _mux_video_audio_with_ffmpeg(
