...
_FREE_VIDEO_SOURCES = {"pexels", "pixabay"}
_MAX_FREE_AUDIO_SECONDS = 360
_MAX_SCRIPT_CHARS = 1500


def _local_script(subject: str) -> str:
    subject = (subject or "").strip()
    if not subject:
        return ""
    return (
        f"{subject}。"
        f"围绕「{subject}」展开介绍，帮助观众快速了解核心要点。"
        f"用简洁画面呈现{subject}，适合短视频传播。"
    )


def _local_terms(subject: str, script: str, amount: int = 5) -> list[str]:
    text = f"{subject or ''} {script or ''}"
    parts = re.split(r"[\s,，。！？!、；;:：\n/\\|]+", text)
    terms = []
    seen = set()
    for part in parts:
...
def _local_social_metadata(video_subject: str, video_script: str) -> dict:
    title = (video_subject or "Short Video").strip()[:80]
    caption = (video_script or video_subject or "").strip()[:200]
    tags = _local_terms(video_subject, video_script, 5)
    hashtags = [tag.replace(" ", "") for tag in tags if tag][:5]
...
def generate_script(task_id, params):
    ...
    if not video_script:
        video_script = _local_script(params.video_subject)
        logger.debug(f"local video script: \n{video_script}")
    else:
    ...
    if len(video_script) > _MAX_SCRIPT_CHARS:
        _mark_task_failed(
            task_id,
            "script",
            f"script is too long ({len(video_script)} chars); keep it under {_MAX_SCRIPT_CHARS} characters for about {_MAX_FREE_AUDIO_SECONDS}s of video",
        )
        return None
...
def generate_terms(task_id, params, video_script):
    ...
    if not video_terms:
        ...
        video_terms = _local_terms(params.video_subject, video_script, amount)
        logger.debug(f"local video terms: {utils.to_json(video_terms)}")
    else:
...
def get_video_materials(
    ...
    if params.video_source == "local":
        ...
        source = (
            params.video_source
            if params.video_source in _FREE_VIDEO_SOURCES
            else "pexels"
        )
        ...
        if not downloaded_videos:
            fallback = "pixabay" if source == "pexels" else "pexels"
            logger.info(f"fallback downloading videos from {fallback}")
            downloaded_videos = material.download_videos(
                task_id=task_id,
                search_terms=video_terms,
                source=fallback,
        ...
        if not downloaded_videos:
            _mark_task_failed(
                task_id,
                "materials",
                "failed to download video materials from pexels/pixabay",
            )
            return None
...
def _run_cross_post(
    ...
    try:
        ...
        if platforms:
            ...
            metadata = _local_social_metadata(
...
def _run_pipeline(
    ...
    if audio_duration and audio_duration > _MAX_FREE_AUDIO_SECONDS:
        return _mark_task_failed(
            task_id,
            "audio",
            f"narration is {audio_duration}s; keep the script shorter so the video stays within {_MAX_FREE_AUDIO_SECONDS}s",
        )
