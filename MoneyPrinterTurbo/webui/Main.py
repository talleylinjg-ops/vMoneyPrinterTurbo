...
def _render_generation_controls(
    ...
    if start_button:
        ...
        script_len = len(params.video_script or "")
        if script_len > tm._MAX_SCRIPT_CHARS:
            _remove_active_generation_task(task_id)
            st.error(
                tr("Video Script Too Long").format(
                    chars=script_len,
                    max_chars=tm._MAX_SCRIPT_CHARS,
                    seconds=tm._MAX_FREE_AUDIO_SECONDS,
                )
            )
            st.stop()
